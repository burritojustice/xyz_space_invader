...
<div id="controls_left" class="controls">
  <div id="spaces">
    <p id="space_info">
      {#if space}
        {space.name}: {space.title}<br>
        {space.numFeatures.toLocaleString()} features, {space.dataSize}<br>
        {space.description}
      {/if}
    </p>
    <p id="style_info">
...
<div id="controls_right" class="controls">
  <div id="tag_summary">
    ...
    <p id="and_or">
      <input type="radio" name="and_or" value="or" bind:group='tagFilterAndOr'>or
...
export default {
  ...
  computed: {
    ...
    queryParams: ({ basemap, displayToggles, selectedFeatureProp, tagFilterQueryParam }) => {
  ...
  methods: {
    ...
    resetSpace() {
      this.set({ uniqueTagsSeen: new Set() }); // reset unique tags when loading new space
    },
